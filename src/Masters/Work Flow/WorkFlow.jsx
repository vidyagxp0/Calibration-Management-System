            ...
            <table className='min-w-full bg-white border border-gray-300'>
              ...
              <tbody>
                {workFlows.map((workflow, index) => (
                  <tr key={index}>
                    <td className='p-2 border border-gray-300'>{workflow.id}</td>
                    <td className='p-2 border border-gray-300'>{workflow.groupName}</td>
                    <td className='p-2 border border-gray-300'>{workflow.createdBy}</td>
                    <td className='p-2 border border-gray-300'>{workflow.createdDate}</td>
                    <td className='p-2 border border-gray-300'>{workflow.status.toString()}</td>
                    <td className='p-2 border border-gray-300'>
                      <div className="flex gap-2">
                        <button className="p-1 bg-yellow-500 text-white" onClick={() => handleEditClick(workflow)}>Edit</button>
                        <button className="p-1 bg-red-500 text-white" onClick={() => alert("Are You Sure You Want To Delete?")}>Delete</button>
                      </div>
                    </td>
                  </tr>
                ))}
        ...
        <div className='col-span-2'>
          <div className="p-2 border-l border-gray-400">
            <div className="flex flex-col pt-4 gap-3">
              ...
              <input type="text" value={workflowName} onChange={(e) => setWorkflowName(e.target.value)} className="border border-gray-500 rounded p-2 w-full" />
            </div>
            <div className='flex flex-col md:flex-row justify-around border-b border-gray-400'>
              <div className='flex flex-col py-3'>
                ...
                <select className='p-[10px] text-center border border-gray-500 rounded' value={flowStep} onChange={(e) => setFlowStep(e.target.value)}>
              ...
              <div className='flex flex-col py-3'>
                ...
                <select className='p-[10px] text-center border border-gray-500 rounded' value={employee} onChange={(e) => setEmployee(e.target.value)}>
              ...
              <div className='flex flex-col py-3'>
                ...
                <select className='p-[10px] text-center border border-gray-500 rounded' value={role} onChange={(e) => setRole(e.target.value)}>
                  <option value="Admin">Admin</option>
              ...
              <div className='flex items-center pt-6 md:pt-0'>
                <button className='p-2 bg-green-600 text-white' onClick={addStep}>Add</button>
              </div>
            </div>
                ...
                <tbody>
                  {workflowSteps.map((step, index) => (
                    <tr key={index}>
                      ...
                      <td className='p-2 border border-gray-300'>{step.flowStep}</td>
                      <td className='p-2 border border-gray-300'>{step.employee}</td>
                      <td className='p-2 border border-gray-300'>{step.role}</td>
                      <td className='p-2 border border-gray-300'>
                        <button className="p-1 bg-red-500 text-white" onClick={() => {
            ...
            <button className="bg-green-600 text-white w-full mt-3 py-2" onClick={isEditing ? updateWorkflow : addWorkflow}>
